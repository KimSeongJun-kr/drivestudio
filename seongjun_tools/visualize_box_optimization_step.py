...
def add_ego_pose_to_checkpoint_boxes(nusc: 'NuScenes', 
...
def _add_checkpoint_boxes_to_geometries(frame_boxes: Dict[int, List], 
                                      frame_id: int,
                                      color_mapping: Dict[str, Tuple[float, float, float]],
                                      use_ego_coordinates: bool = True) -> List:
    """체크포인트 박스들을 기하학적 객체로 변환합니다."""
    geometries = []
...
# ===========================
# 체크포인트 로딩 및 애니메이션 함수들
# ===========================

def find_checkpoint_files(checkpoint_dir: str) -> List[Tuple[int, str]]:
    """체크포인트 디렉토리에서 체크포인트 파일들을 찾고 iteration 순서대로 정렬합니다."""
    checkpoint_files = []
    
    # checkpoint_*.pth 패턴으로 파일 검색
    pattern = os.path.join(checkpoint_dir, "checkpoint_*.pth")
    files = glob.glob(pattern)
    
    for file_path in files:
        filename = os.path.basename(file_path)
        # checkpoint_02000.pth, checkpoint_80000_final.pth 등에서 숫자 추출
        match = re.search(r'checkpoint_(\d+)', filename)
        if match:
            iteration = int(match.group(1))
            checkpoint_files.append((iteration, file_path))
    
    # iteration 순서로 정렬
    checkpoint_files.sort(key=lambda x: x[0])
    
    print(f"📁 찾은 체크포인트 파일들 ({len(checkpoint_files)}개):")
    for iteration, file_path in checkpoint_files:
        print(f"  - Iteration {iteration:06d}: {os.path.basename(file_path)}")
    
    return checkpoint_files

def check_checkpoint_structure(checkpoint: dict) -> dict:
...
def extract_rigid_nodes_boxes(models: dict, frame_boxes: dict, node_type: str = 'RigidNodes', camera_front_start: Optional[np.ndarray] = None):
    """RigidNodes에서 박스 정보 추출"""
    rigid_quats = models['RigidNodes']['instances_quats'].numpy()
    rigid_trans = models['RigidNodes']['instances_trans'].numpy()
    rigid_sizes = models['RigidNodes']['instances_size'].numpy()
    
    num_frames, num_instances = rigid_quats.shape[:2]
    
    # 5의 배수 프레임만 처리 (NuScenes 키프레임)
    for frame_id in range(0, num_frames, 5):
        if frame_id not in frame_boxes:
        ...
        for instance_id in range(num_instances):
            ...
            # Size 순서 변경 [w, l, h] -> [l, w, h]
            size_corrected = [size[1], size[0], size[2]]
            ...
            frame_boxes[frame_id].append(box_data)

def extract_smpl_nodes_boxes(models: dict, frame_boxes: dict, node_type: str = 'SMPLNodes', camera_front_start: Optional[np.ndarray] = None):
    """SMPLNodes에서 박스 정보 추출"""
    smpl_instance_quats = models['SMPLNodes']['instances_quats'].numpy()  # (num_frames, num_instances, 1, 4)
    smpl_instance_trans = models['SMPLNodes']['instances_trans'].numpy()  # (num_frames, num_instances, 3)
    smpl_sizes = models['SMPLNodes']['instances_size'].numpy()           # (num_instances, 3)
...
def extract_deformable_nodes_boxes(models: dict, frame_boxes: dict, node_type: str = 'DeformableNodes', camera_front_start: Optional[np.ndarray] = None):
    """DeformableNodes에서 박스 정보 추출"""
    deform_quats = models['DeformableNodes']['instances_quats'].numpy()  # (num_frames, num_instances, 4)
    deform_trans = models['DeformableNodes']['instances_trans'].numpy()  # (num_frames, num_instances, 3)
    deform_sizes = models['DeformableNodes']['instances_size'].numpy()   # (num_instances, 3)
...
def extract_all_boxes_from_checkpoint(checkpoint_path: str, camera_front_start: Optional[np.ndarray] = None) -> Optional[Dict[int, List]]:
    """체크포인트 파일에서 모든 프레임의 박스 정보를 추출합니다."""
    try:
        print(f"🔍 체크포인트 로딩: {os.path.basename(checkpoint_path)}")
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        
        if not isinstance(checkpoint, dict) or 'models' not in checkpoint:
            print(f"❌ 유효하지 않은 체크포인트 구조: {checkpoint_path}")
            return None
        ...
        if not structure_check['has_models']:
            print("❌ Error: No 'models' key found in checkpoint.")
            return None
        ...
        if structure_check['has_rigid_nodes']:
            print("Extracting RigidNodes boxes...")
            extract_rigid_nodes_boxes(models, frame_boxes, 'RigidNodes', camera_front_start)
            rigid_count = sum(len([box for box in boxes if box['node_type'] == 'RigidNodes']) for boxes in frame_boxes.values())
            print(f"✅ Extracted {rigid_count} RigidNodes boxes")
            total_extracted += rigid_count
        
        if structure_check['has_smpl_nodes']:
            print("Extracting SMPLNodes boxes...")
            extract_smpl_nodes_boxes(models, frame_boxes, 'SMPLNodes', camera_front_start)
            smpl_count = sum(len([box for box in boxes if box['node_type'] == 'SMPLNodes']) for boxes in frame_boxes.values())
            print(f"✅ Extracted {smpl_count} SMPLNodes boxes")
            total_extracted += smpl_count
        
        if structure_check['has_deformable_nodes']:
            print("Extracting DeformableNodes boxes...")
            extract_deformable_nodes_boxes(models, frame_boxes, 'DeformableNodes', camera_front_start)
            deform_count = sum(len([box for box in boxes if box['node_type'] == 'DeformableNodes']) for boxes in frame_boxes.values())
            print(f"✅ Extracted {deform_count} DeformableNodes boxes")
            total_extracted += deform_count
        
        print(f"✅ 추출된 총 박스 수: {total_extracted}개 ({len(frame_boxes)}개 프레임)")
...
def create_all_sample_animations(checkpoint_dir: str, output_dir: str,
    ...
    if not checkpoint_files:
        print(f"❌ 체크포인트 파일을 찾을 수 없습니다: {checkpoint_dir}")
        return
    
    print(f"🎬 애니메이션 생성 시작: {len(checkpoint_files)}개 체크포인트")
    if show_lidar:
...
def main() -> None:
    ...
    if not os.path.exists(args.checkpoint_dir):
        print(f"❌ 체크포인트 디렉토리를 찾을 수 없습니다: {args.checkpoint_dir}")
        return
    
    if args.output_dir is None:
        args.output_dir = os.path.join(args.checkpoint_dir, "box_optimization_animation")
