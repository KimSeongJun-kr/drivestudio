...
def sample_translation_noise(xyz_std_m: Tuple[float, float, float]) -> np.ndarray:
    std_xyz = np.asarray(xyz_std_m, dtype=np.float64)
    noise = np.random.normal(loc=0.0, scale=std_xyz, size=(3,))
    return noise.astype(np.float64)


def sample_rotation_noise_matrix(rpy_std_deg: Tuple[float, float, float]) -> np.ndarray:
    """
    회전 노이즈 행렬을 샘플링합니다.
    ...
    roll_rad = np.random.normal(loc=0.0, scale=np.deg2rad(roll_std_deg))
    pitch_rad = np.random.normal(loc=0.0, scale=np.deg2rad(pitch_std_deg))
    yaw_rad = np.random.normal(loc=0.0, scale=np.deg2rad(yaw_std_deg))
...
def add_noise_to_transform(transform_4x4: np.ndarray, xyz_std_m: Tuple[float, float, float], rpy_std_deg: Tuple[float, float, float]) -> np.ndarray:
    ...
    rotation_noise = sample_rotation_noise_matrix(rpy_std_deg)
    translation_noise = sample_translation_noise(xyz_std_m)
...
def process_instances(instances_info: Dict[str, Any], xyz_std_m: Tuple[float, float, float], rpy_std_deg: Tuple[float, float, float]) -> Dict[str, Any]:
    for instance_key, instance_val in instances_info.items():
        ...
        for i in range(len(obj_to_world_list)):
            ...
            noisy_m = add_noise_to_transform(m, xyz_std_m, rpy_std_deg)
...
def single_file_process(args: argparse.Namespace) -> None:
    ...
    instances_info_noisy = process_instances(instances_info, xyz_std_m=t_std, rpy_std_deg=r_std)
    ...
    for seq_root in seq_roots:
        ...
        try:
            ...
            with open(in_path, "r") as f:
                instances_info = json.load(f)
            instances_info_noisy = process_instances(instances_info, xyz_std_m=t_std, rpy_std_deg=r_std_batch)
