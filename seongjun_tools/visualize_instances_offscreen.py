...
def visualize_ego_translations_open3d(gaussian_boxes: Optional[EvalBoxes] = None, 
    ...
    # 테스트용 간단한 객체들 추가 (디버깅용)
    test_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=5.0)
    test_sphere.translate([10, 0, 0])
    test_sphere.paint_uniform_color([1, 0, 0])  # 빨간색
    # Unlit 셰이더 사용하므로 법선 벡터 계산 불필요
    geometries.append(test_sphere)
    
    test_cube = o3d.geometry.TriangleMesh.create_box(width=8, height=8, depth=8)
    test_cube.translate([-10, 0, 0])
    test_cube.paint_uniform_color([0, 1, 0])  # 녹색
    # Unlit 셰이더 사용하므로 법선 벡터 계산 불필요
    geometries.append(test_cube)
    
    print(f"🔧 테스트 객체 추가: coordinate frame, sphere, cube")
    ...
    if save_path is not None:
        try:
            ...
            # 이미지 렌더링 및 저장
            success = render_and_save_offscreen(geometries, save_path)
            
            # 오프스크린 렌더링이 실패한 경우 더 간단한 방법 시도
            if not success:
                print("🔄 오프스크린 렌더링 실패. 간단한 방법으로 재시도 중...")
                success = render_and_save_simple(geometries, save_path)
...
def render_and_save_offscreen(geometries, save_path, w=3840, h=2160):
    ...
    try:
        ...
        scene.set_background([1, 1, 1, 1])      # 흰색 배경

                          # 조명 설정 건너뜀 (unlit 셰이더 사용하므로 불필요)
        print("⚠️ Unlit 셰이더 사용으로 조명 설정 건너뜀")
        
                # 재질 설정
        print("🎨 재질 설정 중...")
        mat = rendering.MaterialRecord()
         
                 # 안정성을 위해 unlit 셰이더 사용 (법선 벡터 불필요)
        mat.shader = "defaultUnlit"
        print("✅ Unlit 셰이더 사용 (법선 벡터 불필요)")
        
        # 기본 재질 속성 설정
        mat.base_color = [1.0, 1.0, 1.0, 1.0]  # 기본 흰색

        print(f"🎯 {len(geometries)}개 객체를 장면에 추가 중...")
        valid_objects = 0
        for i, g in enumerate(geometries):
            try:
                ...
                print(f"⚠️ 객체 {i} 추가 실패: {e}")
                continue

        print(f"✅ {valid_objects}/{len(geometries)}개 객체가 성공적으로 추가되었습니다.")
...
def render_and_save_simple(geometries, save_path, w=1920, h=1080):
    """간단한 오프스크린 렌더링을 수행합니다 (최소한의 설정).
    ...
    """
    try:
        print(f"🔄 간단한 오프스크린 렌더링 시작... ({w}x{h})")
        
        # 최소한의 설정으로 오프스크린 렌더러 생성
        renderer = rendering.OffscreenRenderer(w, h)
        scene = renderer.scene
        ...
        for i, g in enumerate(geometries):
            try:
                scene.add_geometry(f"geometry_{i}", g, mat)
                valid_objects += 1
            except Exception as e:
                print(f"⚠️ 객체 {i} 추가 실패: {e}")
                continue
        ...
        print("📹 간단한 카메라 설정...")
        try:
            scene.camera.look_at([0, 0, 0],      # 원점을 바라봄
                            [0, 0, 100],     # 위에서 아래로 보는 위치
                            [0, -1, 0])      # up 벡터
            print("✅ 카메라 설정 완료")
        except Exception as e:
            print(f"⚠️ 카메라 설정 실패: {e}")
...
def _add_boxes_to_geometries(boxes: Optional[EvalBoxes], 
    ...
    for sample_token_iter in sample_tokens_to_process:
        ...
        for box in boxes[sample_token_iter]:
            ...
            geometries.append(create_open3d_sphere(front_center, radius=1.0, color=color))
