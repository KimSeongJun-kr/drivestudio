...
def get_nuscenes_scene_info(dataroot: str, scene_name: str) -> Tuple[str, str, List[str]]:
...
def extract_rigid_nodes_poses(checkpoint: Dict[str, Any], scene_token: str, scene_name: str, sample_tokens: List[str] = None) -> List[PoseAnnotation]:
    ...
    for instance_id in range(num_instances):
        ...
        for idx, frame_id in enumerate(keyframe_indices):
            ...
            translation = rigid_trans[frame_id, instance_id].tolist()
            rotation = rigid_quats[frame_id, instance_id].tolist()
...
def extract_smpl_nodes_poses(checkpoint: Dict[str, Any], scene_token: str, scene_name: str, sample_tokens: List[str] = None) -> List[PoseAnnotation]:
    ...
    for instance_id in range(num_instances):
        ...
        for idx, frame_id in enumerate(keyframe_indices):
            ...
            translation = smpl_instance_trans[frame_id, instance_id].tolist()
            rotation = smpl_instance_quats[frame_id, instance_id, 0].tolist()  # (1, 4) -> (4,)
...
def extract_deformable_nodes_poses(checkpoint: Dict[str, Any], scene_token: str, scene_name: str, sample_tokens: List[str] = None) -> List[PoseAnnotation]:
    ...
    for instance_id in range(num_instances):
        ...
        for idx, frame_id in enumerate(keyframe_indices):
            ...
            translation = deform_trans[frame_id, instance_id].tolist()
            rotation = deform_quats[frame_id, instance_id].tolist()
...
def extract_all_poses_from_checkpoint(checkpoint_path: str, output_dir: str, nuscenes_dataroot: Optional[str] = None, scene_name: str = "") -> None:
    ...
    if nuscenes_dataroot and scene_name:
        scene_token, scene_name, sample_tokens = get_nuscenes_scene_info(nuscenes_dataroot, scene_name)
        if sample_tokens:
    ...
    if structure_check['has_rigid_nodes']:
        ...
        try:
            rigid_annotations = extract_rigid_nodes_poses(checkpoint, scene_token, scene_name, sample_tokens)
    ...
    if structure_check['has_smpl_nodes']:
        ...
        try:
            smpl_annotations = extract_smpl_nodes_poses(checkpoint, scene_token, scene_name, sample_tokens)
    ...
    if structure_check['has_deformable_nodes']:
        ...
        try:
            deform_annotations = extract_deformable_nodes_poses(checkpoint, scene_token, scene_name, sample_tokens)
...
def main():
    ...
    extract_all_poses_from_checkpoint(args.checkpoint, args.output, args.nuscenes_dataroot, args.scene_name)
