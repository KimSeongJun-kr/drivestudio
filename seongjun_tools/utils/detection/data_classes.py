...
class DetectionMetrics:
    ...
    def __init__(self, cfg: DetectionConfig):
        ...
        self._label_tp_MAE_errors = defaultdict(lambda: defaultdict(float))
        self._label_tp_RMSE_errors = defaultdict(lambda: defaultdict(float))
        self.eval_time = None
    ...
    def add_label_tp(self, detection_name: str, metric_name: str, tp: float):
        self._label_tp_MAE_errors[detection_name][metric_name] = tp

    def add_label_tp_RMSE(self, detection_name: str, metric_name: str, tp: float):
        self._label_tp_RMSE_errors[detection_name][metric_name] = tp

    def get_label_tp(self, detection_name: str, metric_name: str) -> float:
        return self._label_tp_MAE_errors[detection_name][metric_name]
    
    def get_label_tp_RMSE(self, detection_name: str, metric_name: str) -> float:
        return self._label_tp_RMSE_errors[detection_name][metric_name]
    ...
    def tp_errors(self) -> Dict[str, float]:
        ...
        for metric_name in TP_METRICS:
            class_errors = []
            for detection_name in self._label_tp_MAE_errors.keys():
    ...
    @property
    def tp_RMSE_errors(self) -> Dict[str, float]:
        """ Calculates the mean true positive error across all classes for each metric. """
        errors = {}
        for metric_name in TP_METRICS:
            class_errors = []
            for detection_name in self._label_tp_RMSE_errors.keys():
                class_errors.append(self.get_label_tp_RMSE(detection_name, metric_name))
    ...
    def serialize(self):
        return {
            ...
            'label_tp_MAE_errors': self._label_tp_MAE_errors,
            'label_tp_RMSE_errors': self._label_tp_RMSE_errors,
            'tp_errors': self.tp_errors,
            'tp_RMSE_errors': self.tp_RMSE_errors,
    ...
    def deserialize(cls, content: dict):
        ...
        for detection_name, label_tps in content['label_tp_MAE_errors'].items():
        ...
        for detection_name, label_tps in content['label_tp_RMSE_errors'].items():
            for metric_name, tp in label_tps.items():
                metrics.add_label_tp_RMSE(detection_name=detection_name, metric_name=metric_name, tp=float(tp))
    ...
    def __eq__(self, other):
        ...
        eq = eq and self._label_tp_MAE_errors == other._label_tp_MAE_errors
        eq = eq and self._label_tp_RMSE_errors == other._label_tp_RMSE_errors
